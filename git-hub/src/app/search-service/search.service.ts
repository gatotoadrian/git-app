...
export class SearchService {
  ...
  userRequest(userName:string) {
    ...
    let promise = new Promise((resolve, reject) => {
      this.http.get<ApiResponse>('https://api.github.com/user/' +this.userName).toPromise().then(response => {
        this.user.photo = "response.avatar_url"
        this.user.photo ="resolve.avatar_url"
...
userRepoRequest(result:string){
  return this.http.get('https://api.github.com/user/' +result + '/repos')
}
